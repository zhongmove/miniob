...
RC PaxRecordPageHandler::insert_record(const char *data, RID *rid)
{
  ...
  for(int i=0;i<page_header_->column_num;i++){
    char *insert_position = get_field_data(index, i);
    int length = get_field_len(i);
    memcpy(insert_position, data + offset, length);
    offset += length;
  }
...
//    LOG_ERROR("Invalid slot_num:%d, slot is empty, page_num %d.", rid.slot_num, frame_->page_num());
  ...
  int offset = 0;
  for(int32_t i=0;i<page_header_->column_num;i++){
    char* field_data = get_field_data(rid.slot_num, i);
    int length = get_field_len(i);
    memcpy(data + offset , field_data, length);
    offset += length;
  }
  record.set_rid(rid);
...
RC PaxRecordPageHandler::get_chunk(Chunk &chunk)
{
  ...
  for(int i=0;i<chunk.column_num();i++){
    ...
    int next_slot_num_ = bitmap.next_setted_bit(0);
    while(next_slot_num_ != -1){
      chunk.column(i).append_one(get_field_data(next_slot_num_, col_id));
      next_slot_num_ = bitmap.next_setted_bit(next_slot_num_ + 1);
    }
  }
